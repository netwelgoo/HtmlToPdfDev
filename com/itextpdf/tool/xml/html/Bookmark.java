...
import com.itextpdf.text.BookmarkWritableDirectElement;
...
/**
 * @author Emiel Ackermann, redlab_b, pioneer(2016.02.19)
 * @comment 
 *    example 1. <bookmark title="abc", level=1 />  : bookmark title= "abc"
 *            2. <bookmark level=1>bbb</bookmark>   : bookmark title="bbb" 
 *            3. <bookmark title="xxx" level=1>yyy</bookmark>   : bookmark title="xxx"
 */
public class Bookmark extends AbstractTagProcessor {
...
//FIXME pioneer(2016.03.17)					
				l.add(new BookmarkWritableDirectElement() {
					public void write(final PdfWriter writer, final Document doc) throws DocumentException {
						PdfDestination destination = new PdfDestination(PdfDestination.XYZ, 20,	writer.getVerticalPosition(false), 0);
						...
						String bookmarkTitle = (getTitle(tag).equals("")) 
												? title.toString()
												: getTitle(tag);
//							String bookmarkTitle = getTitle(tag);
						
						if (null == tree) {
							// first bookmark tag encounter
						...
						if (LOGGER.isLogging(Level.TRACE)) {
							LOGGER.trace(String.format(LocaleMessages.getInstance().getMessage(LocaleMessages.ADD_HEADER), bookmarkTitle));
						}
						HeaderNode node = new HeaderNode(level, new PdfOutline(tree.outline(), destination, bookmarkTitle), tree);
						memory.put(HtmlPipelineContext.BOOKMARK_TREE, node);
					}
				});
//				}
				context.autoBookmark(true);
	...
	private int getLevel(final Tag tag) {
		...
		return Integer.parseInt(attributes.get("level").trim().replaceAll("\"", ""));
	}
	
	/**
	* 
	* @param tag bookmark title
	* @return
	 */
	private String getTitle(final Tag tag) {
		Map<String, String> attributes = tag.getAttributes();
		if(attributes.get("title") == null) return "";
		return attributes.get("title").trim();
	}
