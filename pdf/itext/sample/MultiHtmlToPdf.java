...
public class MultiHtmlToPdf {
	
	public void htmlToPdf(String text, String i) throws Exception{
		Document document = new Document(PageSize.A4, 50, 50, 50, 50); // 용지 및 여백 설정

// PdfWriter 생성
		PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("C:/project/PDF/workspace/FOP/sample/out/s/shinhanText."+i+".pdf"));
		...
		for (int i = 0; i < 10; i++) {
			new Thread(j+"_"+i){
				...
				//		BufferedReader reader = new BufferedReader(new FileReader("C:/project/PDF/sample/html/letter1.html"));
						...
						new MultiHtmlToPdf().htmlToPdf(buffer.toString(), this.currentThread().getName());
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
